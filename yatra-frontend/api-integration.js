...
function getAuthToken() {
    try {
        const userData = localStorage.getItem('travelerData');
        if (userData) {
...
const api = {
    ...
    async fetch(endpoint, options = {}) {
        // If no API_BASE configured, skip API calls (use localStorage only)
        if (!API_BASE) {
            throw new Error('API_BASE not configured - using localStorage fallback');
        }
